...
Write the people as a casting range, never as a character. One description wide enough that twenty pictures cast twenty different people who all belong, and particular enough that someone outside this world plainly would not. This is not a workaround for anything. There are millions of people in a casting range and a brand's pictures should look like it, so the range is the right answer and would be even if you could hold one face across every frame. If you want to name someone to make the description concrete, mark it as an example and write it so a reader knows it is one of many: nobody recurs across this brand's pictures, and a named person in a denim jacket turning up in every frame is the failure on the other side of a vague description.

Give the world its range of scale. A world is not only rooms. It runs from one person alone to a few people together to a crowd of thousands, and the same territory holds at every size: a festival field, a packed room, a street at closing time, and a kitchen at seven in the morning can all belong to one world. Name settings at more than one scale, because a world written entirely in small interiors produces twenty pictures of tables.
