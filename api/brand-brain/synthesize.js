...
// Synthesis runs in eight ordered passes, one request each: four for the brand
// today (2026-09-07) and four for the brand evolved (ADR 0019, 2026-09-09).
// The body carries `pass`; pass 1 carries the sources and the mode, and passes
// 2 through 8 carry only the request id, because the server holds the
// in-progress brain between them. Passes 5 through 8 also carry `reach`, the
// level the evolved world is built at. Eight calls inside one request would
// share one 300 second clock and time out the way the single call did, only
// later. See docs/findings-2026-09-07-four-pass-synthesis.md.
