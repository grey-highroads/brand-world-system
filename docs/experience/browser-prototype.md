...
4. Add a required usage instruction, guidance area, influence where interpretation is appropriate, and optional exclusions. Protected assets and approved guidance are not influence-weighted. File intake shows accepted formats, enforces 20 MB per file and 40 MB per synthesis batch, and does not accept folders or mixed multi-file records. Approved guidance can be a supported document, structured text file, or PNG, JPG, or WebP page image. Multi-page books work best as PDF; SVG and native design files require conversion when the system needs to interpret them.
