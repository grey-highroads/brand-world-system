...
- Delta from this commit, the render section catches up with two engines and a two-call path, and a discard stops leaving a picture behind. Two commits changed `src/production/service.js`, a module this contract covers, without updating it in the same commit. `b6782b5` added Seedream as a second render engine behind an owner-facing picker; `af44448` split the Seedream render in two when the job locks an asset. That is the maintenance rule violated a third and a fourth time, recorded here rather than smoothed over, and stages 9 and 10 below are rewritten from the tree rather than amended from the old text. This commit also changes `src/production/store.js` and adds `test/production-store.test.js`. Nothing on the render path moves.
  ...
  **The scene prompt is the same compiler with one argument dropped.** `prepareProductionPackage` compiles once into `generationPackage`, then, when the engine resolves to `seedream` and a locked asset is present, compiles a second time with `lockedAsset: null` and keeps only that package's prompt string (L394 to L407). That is the shape the compiler already produces for a job that locks nothing, so there is no second prompt to keep in step with the first. The locked asset stays on the package and on the record, because the job did lock one.
...
- **The scene prompt is never a second prompt to maintain.** It is the same compiler with the locked asset withheld, which is the shape a job that locks nothing already produces (L396). A fork here would put two prompt bodies in step with each other forever.
