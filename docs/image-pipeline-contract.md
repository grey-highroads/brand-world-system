...
- Verified against commit: `4f86439da4de6a6fd126d29a59ffca09df25ede0`, the head this update was read against. The maintenance rule asks for this commit; a commit cannot carry its own SHA, so the parent is named and the deltas this commit introduces are stated below.
...
16. **ADR 0017 step 1 exists and nothing consumes it.** `src/refusals/store.js` holds the per-client refusals document and its lifecycle: propose, accept, decline, retire, observe, supersede. No endpoint reaches it, no synthesis path writes to it, no client has a `refusals.json`, and the compile path still reads `livedWorld.rejects` through `package.js#rejectsDirection`. The consumption switch is ADR 0017 step 4 and is gated on document presence per client, the same transition rule ADR 0016 uses. Two departures from the claims store pattern are deliberate and recorded in `docs/evaluations/2026-08-17-adr-0017-step1-refusals-gate.md`: the lifecycle is a pure document layer with the blob factory composed on top, so the gate can round-trip fixtures without the network, and deletion is absent, with retirement as a named status rather than an absence. The hand-authored fixtures under `fixtures/adr-0017-refusals/` are prototype material, not client state.
