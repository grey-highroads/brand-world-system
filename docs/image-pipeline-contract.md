...
- Verified against commit: `381d3b7231c294a96878841d6a6387097a13307e`, the head this update was read against. The maintenance rule asks for this commit; a commit cannot carry its own SHA, and the anchors below are true at `381d3b7`, so the parent is named and the one delta is stated in the next bullet.
- Delta since the verified commit: this commit deleted `src/production/package.js#compileProductSection` and its comment (L334 to L361 at `381d3b7`), retiring known ambient state 4. Every `package.js` anchor below L334 in this document therefore reads 28 lines high against head; anchors in every other module are unchanged. Re-anchoring is outstanding work, stated here rather than left silent.
