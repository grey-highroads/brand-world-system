...
- Verified against commit: `f8432cb0f867b54c5b6225ec01eca01722897dae`, the head this update was read against. The maintenance rule asks for this commit; a commit cannot carry its own SHA, so the parent is named and the deltas this commit introduces are stated below.
...
- Delta since the verified commit: this commit makes the visual grammar artifact reviewable without real data. `app/app.js` gains `grammarAbsentNote`, `renderGrammarSample`, a `brain-grammar-sample` screen, and a read-only mode on `renderGrammarArtifact` that suppresses the comment affordance so a sample cannot write into a real client's stored feedback; `app/styles.css` gains the two blocks those surfaces use; `docs/ui-contribution-guide.md` gains the presence-state testing rule. Every `app.js` anchor below L3300 in this document reads low against head by roughly 60 lines, on top of the drift already stated below. No compile-path behavior changes.
