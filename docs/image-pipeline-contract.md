...
- Verified against commit: `6cb4ffe0a09465d25e96c198fe6ef107ecbe0735`, the head this update was read against. The maintenance rule asks for this commit; a commit cannot carry its own SHA, so the parent is named and the deltas this commit introduces are stated below.
...
- Delta since the verified commit: this commit lands the ADR 0016 step 3 synthesis instructions. `src/brand-brain/chat-completions-provider.js` gains a Visual Grammar block in `SYSTEM_INSTRUCTIONS` and one global prose rule banning em dashes, en dashes, and curly punctuation in every generated field. `SYSTEM_INSTRUCTIONS` grows from roughly L5 to L52 to roughly L5 to L100, so every anchor in that file below L52 reads low against head; anchors in every other module are unchanged. No compile-path behavior changes, and no synthesis has been run against the new instructions at the time of this commit.
