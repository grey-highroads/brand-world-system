...
- Verified against commit: `d2a5fcfd2fd98fe3c41f9d535fecbc5fc014b15d`, the head this update was read against. The maintenance rule asks for this commit; a commit cannot carry its own SHA, so the parent is named and the deltas this commit introduces are stated below. Anchors in stages 9, 10, and 11 were re-read against that tree in this update and are current. Anchors everywhere else in the document were not re-read and still carry the drift the earlier header entries describe: anchors in `src/production/package.js`, `src/production/prompt-craft.js`, and `src/production/looks.js` read low against head by roughly 30 to 45 lines, and anchors in `src/production/service.js` outside stages 9 and 10 read low by an amount that grows down the file, spot measured at roughly 25 lines at `resolveProduct`, roughly 55 at the drafted-copy block, and roughly 90 at `readProductionJob`. Every named function and constant is still present under its stated name.
...
- Delta from this commit, the render section catches up with two engines and a two-call path, and a discard stops leaving a picture behind. Two commits changed `src/production/service.js`, a module this contract covers, without updating it in the same commit. `b6782b5` added Seedream as a second render engine behind an owner-facing picker; `af44448` split the Seedream render in two when the job locks an asset. That is the maintenance rule violated a third and a fourth time, recorded here rather than smoothed over, and stages 9 and 10 below are rewritten from the tree rather than amended from the old text. This commit also changes `src/production/store.js` and adds `test/production-store.test.js`. Nothing on the render path moves.

  **What the engine table does, and what it deliberately does not do.** `src/production/service.js` holds `renderEngines` (L17 to L32) with two entries, `openai` and `seedream`, each carrying a label, a model, an endpoint chooser, a render function, and the name of the environment variable holding its key. `resolveRenderEngine` (L34 to L43) reads `body.engine`, matches own entries only, and falls back to `openai` for anything it does not recognise, so a stale client naming an engine this build has never heard of still gets an image. The compiled prompt is identical either way. The table decides which model receives the prompt and never what the prompt says, which is stated in the comment above it (L11 to L15) and is the line that keeps a second engine from becoming a second compiler.

  **The render is one call, except on one combination.** OpenAI renders in one call, with or without a locked asset. Seedream renders in one call when the job locks nothing. Seedream with a locked asset renders in two: the scene first with the asset withheld, then an edit that places the real product into that render. The reason is recorded in `docs/findings-2026-09-02-two-call-seedream.md` and in the comment at service L382 to L393: a single edit call carrying the product as a reference returns the product oversized, because the reference fills its own frame and that framing carries into the generated scene.

  **The scene prompt is the same compiler in a second mode.** `prepareProductionPackage` compiles once into `generationPackage`, then, when the engine resolves to `seedream` and a locked asset is present, compiles a second time with `lockedAsset: null` and `scenePass: true` and keeps only that package's prompt string (service L403; the scene pass was added after this delta was written and is described in the delta below). That is the shape the compiler already produces for a job that locks nothing, so there is no second prompt to keep in step with the first. The locked asset stays on the package and on the record, because the job did lock one.

  **The record of a two-call render.** `generationPackage.twoCall` carries `engine`, `model`, `scenePrompt`, `sceneEndpoint`, `placementInstruction`, `placementEndpoint`, and, filled in after the images exist, `sceneImageId` and `scenePathname` (L397 to L406, L603 to L612). Both prompts are on it: the scene prompt is the compiled scene-only prompt, and the placement instruction is fixed text from `productPlacementInstruction` (L205 to L207), one sentence asking for the can in the scene to be replaced with the supplied can image. It names no product: the noun is hardcoded to "can" while cans are the test subject, and the `productName` argument is accepted and ignored. Both endpoints are on it: `sceneEndpoint` is chosen from the template and creative references only, so a job carrying neither records the text-to-image endpoint, and `placementEndpoint` is always the edit endpoint, since call two always carries two images. Both images are on it: the finished render at the job's own path and the scene at `{jobId}-scene`. The job record's top-level `endpoint` field is chosen from every reference including the locked asset (L520), so on a two-call job it names the edit endpoint, which is call two's endpoint and not call one's. `twoCall` is set during package preparation, so a preflight response for the same request shows the scene prompt before anything renders.

  **The discard fix.** `deleteOutputArtifacts` removed the job's `output.png` and its `package.json` and knew nothing about `{jobId}-scene/output.png`, so discarding a two-call output left one image in storage that no surface could reach and nothing would ever clean up. Recorded as a finding in `af44448` because `src/production/store.js` was outside that commit's file list. Both store implementations now delete the scene image with the rest. In the Blob store the three paths come from one new function, `outputArtifactPathnames`, and each delete swallows its own failure exactly as the two existing deletes already did, which is what makes it safe to ask for a scene image on the great majority of jobs that never wrote one. The file store adds the scene image to the list it unlinks, and its loop already treats a missing file as done.

  **One duplication, stated rather than hidden.** The `-scene` suffix is written out in `src/production/service.js` where the scene is saved and in `src/production/store.js` where it is deleted. `service.js` imports `store.js`, so the constant cannot travel the other way without inverting that, and inverting it for a suffix is not worth the change. `store.js#sceneImageJobId` carries a comment saying so, and `test/production-store.test.js` asserts the value it returns, so moving one file without the other fails a test rather than silently orphaning an image on every discard from that point on.

- Delta from this commit, the scene call describes a plain product and the client stops abandoning renders that are still working. `src/production/package.js` gains a `scenePass` compile mode, `src/production/service.js` sets it on the scene compile, and `app/app.js` changes how long recovery waits and what decides the failure state. Evidence and reasoning are in `docs/findings-2026-09-02-scene-placeholder-and-recovery.md`.

  **The scene pass is a compile mode, not a fork.** `compileBrandWorldImagePackage` takes `scenePass`, default false (L439). When it is true and the job names a product, the Product knowledge section body is the placeholder from `sceneProductPlaceholder` (L414) instead of `compileProductSectionForImage` (L642). That is the only difference between the two compiles: the scene pass asks in one sentence for a plain unmarked can at its real size, and names no product, since the noun is hardcoded to "can" while cans are the test subject. A job that names no product compiles an empty body, as before. Nothing else in the compiler reads the flag, and at its default every compiled package is unchanged in every byte, checked against head across the four frozen fixture scene shapes locked and unlocked.

  **Why the placeholder exists.** The scene call's product is replaced by the placement call, so it never needed label instructions. Carrying them cost scale: the product record's visual direction asked for the vertical branding and the flavor, energy, caffeine-free, and volume statements to be visible, and the model made the product large enough to carry legible text. The 2026-09-02 job rendered the can at roughly half the frame with no reference image attached at all.

  **Recovery waits for the render, not for the connection.** `recoverProductionJob` (app.js L8072) polls the current-job endpoint every 2500 ms until the job reports `complete` or `error`, with an eight minute ceiling (L8069 to L8070). It previously polled twenty times at 1.5 seconds and gave up after thirty seconds, which failed a four minute two-call render that finished three minutes after the screen said it had not. A dropped generate connection sets no error state on its way into recovery; only an exhausted recovery does, and it marks the job record it holds as errored so the screen has one status to read (L8138).

  **The failure state follows the job's own status.** The result screen derives `failed` from `job.status === "error"`, falling back to the local status only when there is no job at all (app.js L6991). The mixed screen of 2026-09-02, error text beside a Working badge, is unreachable from that expression.

  **Tests.** `test/production-store.test.js` is new and covers the file store against a temporary directory: a two-call job loses its image, its package, and its scene image; a single-call job loses its two artifacts and does not fail on the scene path it never wrote; a discard of a job that was already discarded stays quiet. The Blob store's deletes go through a client library this suite does not stand up, so what is checked there is the list of paths it hands that library, which is where the scene image was missing. Full suite before and after this commit: 152 tests, one failing, `fixtures/copy-audit-mechanism-test.mjs`, which needs an API key and is ambient state 10.
...
`src/production/service.js#generateProductionImage` (L468), reached from `api/production/generate.js#handler` when the user confirms generation after preflight. Before the render is reached, the duplicate-invocation guards run (stage 10's invariants cover the record semantics): a complete record for the same job id returns immediately (L471); a working record younger than five minutes turns this invocation into a poller that waits up to 200 seconds and returns the original's result (L475 to L487, intervals at L458 to L460); a working record older than five minutes is treated as abandoned and re-rendered.

### Inputs

The compiled package's prompt string (`generationPackage.prompt`, L581); up to ten reference images loaded from Blob as bytes, ordered template asset first, locked asset second, then creative references (L496 to L508, L530 to L539); the selected engine's API key from `options.env`, `OPENAI_API_KEY` for OpenAI and `FAL_KEY` for Seedream (L23, L30, read at L542); size from `generationPackage.output.size` via `imageSizeForFormat` (stage 7); quality fixed at `"medium"` and output format `"png"` (L545 to L546). The reference ceiling of eight applies to creative references at resolution time (`#resolveReferences`, L72); the template and locked asset ride above that count. On the two-call path the inputs split: call one takes the scene prompt and every reference except the locked asset, call two takes the fixed placement instruction, the scene render as bytes, and the locked asset (L552 to L577).

### Owner

The engine table in `src/production/service.js` (`renderEngines`, L17 to L32; `resolveRenderEngine`, L34 to L43) selects one of two renderers from `body.engine`, matching own entries only and falling back to `openai` for anything unrecognised. `src/renderers/openai-images.js#renderWithOpenAIImages` (L53), with endpoint selection in `#chooseOpenAIImageEndpoint` (L17 to L19) and request construction in `#buildOpenAIImageGenerationRequest` (L21 to L33) and `#buildOpenAIImageEditRequest` (L35 to L51). `src/renderers/seedream-images.js#renderWithSeedreamImages` (L119), with endpoint selection in `#chooseSeedreamImageEndpoint` (L57 to L59) and request construction in `#buildSeedreamTextToImageRequest` (L75 to L81) and `#buildSeedreamEditRequest` (L83 to L93). The decision to render in one call or two is the service's, not a renderer's: both renderers take a prompt and images and know nothing about the other call.

### Transformations

**Engine selection.** `body.engine` names `openai` or `seedream`; anything else resolves to `openai`. The compiled prompt is identical for either, and the table carries only which model receives it (comment L11 to L15). OpenAI's model is `gpt-image-2` (openai-images.js L1); Seedream's is `bytedance/seedream/v5/pro` (seedream-images.js L16).

**Endpoint selection, per engine, mechanical from reference presence.** Any reference images route to the provider's edit endpoint, none routes to its text-to-image or generations endpoint (openai-images.js L17 to L19; seedream-images.js L57 to L59). OpenAI's edit call is multipart form data with each image appended as `image[]`; Seedream's is JSON carrying each reference as a base64 data URI under `image_urls`, because the source blobs are private and fal's runner fetches input URLs without credentials (seedream-images.js L42 to L45, L83 to L93).

**Call count.** One call on OpenAI, with or without a locked asset. One call on Seedream when the job locks nothing. Two sequential calls on Seedream when the job locks an asset (L552 to L584). The split exists because a single Seedream edit call carrying the product as a reference returns the product oversized: the reference fills its own frame and that framing carries into the generated scene (comment L382 to L393; hand runs of 2026-09-01 and 2026-09-02 recorded in `docs/findings-2026-09-02-two-call-seedream.md`).

**The two calls, in order.** Call one renders the scene from `twoCall.scenePrompt`, which is the same compiler run a second time with `lockedAsset: null` and `scenePass: true` (service L403), carrying the template and creative references and withholding the locked asset so its own framing cannot set the scale of the frame (L552 to L561). Empty image data here throws with a scene-specific message (L563). Call two renders from `twoCall.placementInstruction`, one sentence asking for the can in the scene to be replaced with the supplied can image (`productPlacementInstruction`, L205 to L207), and it carries exactly two images in a fixed order: the scene first as the image being edited, the locked asset second as the supplied can image (L566 to L577).

**The prompt passes through unmodified** on both engines and on both calls: each renderer's `requiredPrompt` validates non-empty only (openai-images.js L5 to L8; seedream-images.js `requiredPrompt`). The response is normalised to one shape before it reaches the service: OpenAI's `data[0].b64_json` is used directly, and the Seedream renderer resolves fal's hosted URL or inline data URI to base64 so the service reads one shape from either engine (seedream-images.js L106 to L117). The service decodes base64 to bytes at L564 for the scene and L587 for the finished image.

### Outputs and artifacts

Returns the response object to the service; the service extracts the image bytes. Persists nothing itself. The service owns persistence, including the intermediate scene image on the two-call path (stage 10).

### Invariants

- **The prompt is the compiled package's prompt exactly**; no stage between compilation and the API call may append or rewrite (source: the compiled prompt is the durable record, ADR 0003 and ADR 0006; verified by each renderer's `requiredPrompt` being the only touch). On the two-call path this holds per call: call one's prompt is a compiler output, and call two's is fixed text recorded on the package before the render, not text assembled at call time.
- **The scene prompt is never a second prompt to maintain.** It is the same compiler with the locked asset withheld and the scene pass set, which is the shape a job that locks nothing already produces apart from one section body (service L403; package.js L643). A fork here would put two prompt bodies in step with each other forever, so the scene pass is a mode of the one compiler and reads as one flag in one expression.
- **Endpoint selection is mechanical from reference count, never a user choice** (source: ADR 0005, presets not selectors; openai-images.js L17 to L19, seedream-images.js L57 to L59). The engine itself is a user choice, exposed as an owner-facing picker, and it selects a model rather than a treatment.
- **A second engine is a second model, never a second compiler.** Nothing in the engine table reaches the prompt (comment L11 to L15).
- **The renderers are provider-shaped, not policy-shaped.** No governance logic lives in either; each takes a prompt, images, and options, and returns bytes or throws. Neither knows whether it is call one or call two.

### Failure states

A non-2xx from either provider throws with the API's message and status (openai-images.js L66 to L70; seedream-images.js `errorMessage` and the throw at L133 to L137); a missing API key fails at the renderer rather than silently upstream (openai-images.js L54; seedream-images.js L120). Empty image data throws in the service, with a message naming the engine, and separately for the scene call so a two-call failure says which call failed ("... returned no image data for the scene.", L563; "... returned no image data.", L586). A failure on call one ends the job with no image written; call two is never reached. The service's catch owns what happens next (stage 10).

### Consumers

Stage 10 consumes the bytes; the result screen consumes the job status and, on a two-call job, renders the scene image beneath the finished one (`app/app.js#sceneRenderFigure`); the package record persisted alongside the output is unaffected by render failure.
...
The render succeeding or failing inside `generateProductionImage` (service.js L512 to L703). Separately, the interface persists its output log through POST `/api/production/outputs` (stage 11) after saving or discarding work.

### Inputs

The working job record, the rendered bytes, the compiled package (including the copy contract and, on a two-call render, the `twoCall` block), the intermediate scene bytes when there are any, and the post-render copy production results.

### Owner

`src/production/store.js#createVercelBlobProductionStore` (L127): `write` for the current-job record (L155 to L164), `writeImage` (L165 to L177), `writeOutputPackage` (L189 to L198), `readOutputPackage` (L199 to L201), `deleteOutputArtifacts` (L207 to L211), `readOutputs` and `writeOutputs` (L227 to L239), `outputImageUrl` and `imageUrl` presigners (L134 to L139, L178, L183 to L185). Path construction is shared: `#productionImagePathname` (L22 to L24), `#outputPackagePathname` (L26 to L28), `#sceneImageJobId` (L36 to L38), and `#outputArtifactPathnames` (L44 to L50), which is the one list a discard works from. The write sequence is owned by `generateProductionImage`.

### Transformations

In order, on the success path:

1. The working record (job id, attempt id, status `working`, engine, engine label, model, endpoint, the full compiled package) is written before the render starts (L512 to L523). On a two-call job the recorded `endpoint` is chosen from every reference including the locked asset (L520), so it names the edit endpoint, which is call two's endpoint rather than call one's; call one's endpoint is recorded separately on the package as `twoCall.sceneEndpoint`.
2. After the render, the ownership check: the record is re-read, and if another attempt now owns it, this attempt discards its result rather than overwriting (L592 to L595). Source: the 2026-08-11 duplicate-render incident.
3. The image bytes are written to the per-job deterministic path `brand-world-system/clients/{clientId}/production/jobs/{jobId}/output.png` (`#productionImagePathname`, store L22 to L24; service L597).
4. On a two-call render only, the intermediate scene image is written through the same `writeImage` under the job id `{jobId}-scene`, landing at `.../jobs/{jobId}-scene/output.png`, and its id and pathname are recorded on `twoCall` (service L603 to L612). A failure here is swallowed on purpose: the deliverable already exists and losing the intermediate does not cost it (comment L599 to L602). Because the record and the package hold the same package object, the ids written here reach both.
5. Governed copy is produced after the image, one block per declared type, pre-produced display blocks carried forward rather than regenerated; a block that fails is recorded as a failure with an errored audit rather than silently omitted (service L619 to L658).
6. The package, now carrying produced copy and findings and, on a two-call render, the scene image id and pathname, is written beside the image at `.../jobs/{jobId}/package.json` (`writeOutputPackage`, service L662 to L675); a failure here is swallowed, costing later review, never the image (comment L659 to L661).
7. The complete record is written with `imagePathname` and `imagePublicUrl: null` (L676 to L685).

On the failure path: only the attempt that owns the record may write the error record, so a failing retry cannot mark a succeeded job failed (L687 to L703).

### Outputs and artifacts

Persisted, per client namespace: the current-job record at `production/current.json` (one slot; a new job replaces it); the image at the per-job path; on a two-call render, the scene image at the `{jobId}-scene` path; the package at the per-job path; the output log at `production/outputs.json`, written by the interface through stage 11 with at most 200 entries and every field stripped to the durable minimum (outputs.js L152 to L180). The scene image has no log entry of its own and is reachable only through the id recorded on the package, which is why a discard has to know about it rather than find it.

### Invariants

- **No presigned URL is ever persisted.** `imagePublicUrl` is written null (service L682); the output log stores `hadImage` as a boolean and never a URL (outputs.js L168 to L173); presigned URLs live fifteen minutes and are minted per read (store L134 to L139, comment L179 to L182). Source: the broken-thumbnail class of bugs, closed structurally by stage 11's redirect route.
...
- **Every blob a job writes is on the discard list.** A render that writes a second image writes it under an id derived from the job id, and `#outputArtifactPathnames` (store L44 to L50) is the single list a discard works from, so a new artifact is added in one place rather than in each caller. Source: the scene image orphaned by `af44448` and closed here; covered by `test/production-store.test.js`.
...
- **Copy failure never costs the image** (L619 to L658); package-write failure never costs the image (L662 to L675); scene-image-write failure never costs the image (L603 to L612).
...
Stage 11 serves the persisted image and package; `api/production/current.js#handler` returns the current job with a freshly minted image URL for recovery (`readProductionJob`, service L349 to L355); the interface's `recoverProductionJob` (app.js L8072) uses it when the gateway drops the response mid-render (app.js L8125 to L8141). Recovery polls every 2500 ms until the job reports `complete` or `error`, up to eight minutes, which is above the server's own 300 second `maxDuration`, so a render still running is still waited for.
