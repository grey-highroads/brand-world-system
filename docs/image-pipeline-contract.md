...
- Verified against commit: `ace649bbd14695ff6c045acdbccb4483e20c02ee`, the head this update was read against. The maintenance rule asks for this commit; a commit cannot carry its own SHA, so the parent is named and the deltas this commit introduces are stated below.
...
16. **ADR 0017 step 3 bootstrap is live and the compile path still does not read it.** `src/refusals/store.js` holds the per-client protections document and its lifecycle: propose, accept, decline, retire, observe, supersede. Three actions on `api/brand-brain/index.js` reach it: `read_refusals`, `rule_refusal`, `seed_refusals`. The brain review screen renders proposals with a keep or decline control per entry and shows the derivation each one rests on. **The compile path is untouched:** `package.js#rejectsDirection` still reads `livedWorld.rejects`, and the switch on document presence is ADR 0017 step 4.
    ...
    **The blob path was Reasoned until its first real write.** The store's read and write were modeled on the claims store and no blob call had run through them. The first seed is that call. Its outcome is recorded in the step 3 note in this contract once it happens; until that line says otherwise, treat the path as unconfirmed.
