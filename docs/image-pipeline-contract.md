...
- Verified against commit: `efe648141444edc36d7bc268e36a44304ade4927`, the head this update was read against. The maintenance rule asks for this commit; a commit cannot carry its own SHA, so the parent is named and the deltas this commit introduces are stated below.
...
- Delta since the verified commit: this commit lands the ADR 0016 step 2 schema. `src/brand-brain/schema.js` gains the `visualGrammar` artifact, the `ambition` origin, a `grammar` reader id, and per-property `description` guidance; `src/brand-brain/chat-completions-provider.js` gains one Lived World sentence fencing `ambition` off until step 3. Every `schema.js` anchor below L43 in this document therefore reads low against head by the size of the insertion, and stage 1's transformation 5 is rewritten below to describe the new state rather than the old anchors. No compile-path behavior changes: stages 6 through 12 are untouched.
...
8. **ADR 0016 is partly implemented: the schema exists, nothing reads it.** Step 2 landed the `visualGrammar` artifact and the `ambition` origin in `schema.js`. Steps 3 and 4 have not landed, so the artifact is synthesized and stored but nothing surfaces or consumes it. Specifically at head: the artifact reader builds its tab list from three entries (`app.js#applySynthesisResult`), so `visualGrammar` renders nowhere; `renderBrainArtifactReader` selects a body renderer with a ternary whose final branch is `renderStoryArtifact`, so a fourth id added without a branch would render as a story artifact and throw on the missing `moments`; `basisNote` (app.js L3036 to L3045) tests `origin === "inference"` and labels everything else "From your sources", so an `ambition` entry would be presented to the reviewer as coming from their own sources, which is the honesty failure the origin exists to prevent; `sampleBrainArtifacts` derives the SLAKE sample from three entries (app.js L981), so the sample brain has no grammar. `livedWorld.rejects` remains the live image-path avoid-clause source (`#rejectsDirection`), and no render evaluation examines the returned image. The three app-side defects are the read-path work gated behind owner review of the schema, and none of them is reachable by a user until that work lands, because nothing displays the artifact.
