...
- Verified against commit: `978b54f1cb5bbf7b57f2a72a3c08952b23cbd73f`, the head this update was read against. The maintenance rule asks for this commit; a commit cannot carry its own SHA, so the parent is named and the deltas this commit introduces are stated below.
...
- Delta since the verified commit: this commit adds the synthesis dry run and the pre-destruction backup. `src/brand-brain/service.js` accepts `dryRun` on the body and returns the computed snapshot without writing, and takes a timestamped backup before any non-incremental write when a stored payload exists, refusing the synthesis if that backup fails. `src/brand-brain/store.js` gains `writeBackup` on both implementations, refusing overwrite. `docs/deferred-work.md` gains the candidate-not-erase entry. Every `service.js` anchor below L55 reads low against head by roughly 30 lines and every `store.js` anchor below L25 by roughly 20; anchors in every other module are unchanged. No compile-path behavior changes.
...
8. **ADR 0016 step 2 is complete and nothing consumes the artifact.** The `visualGrammar` artifact is synthesized by stage 1, stored in the brain snapshot, readable in the Brand Brain artifact reader as a fourth tab, and reviewable without real data through the documented sample path below. No production stage reads it. Specifically at head: the tab is gated on artifact presence (`applySynthesisResult`), so a brain synthesized before the schema shows three tabs and cannot reach a reader that would throw; those brains instead get a plain note naming what visual grammar is and offering the sample. `renderGrammarSample` renders the SLAKE sample's artifact on its own screen, read-only, with the sample brand named in a banner and in the header, mutating no state and saving nothing, which is what keeps it from being mistakable for real data on a shared screen. `basisNote` labels `evidence` as "From your sources", `inference` as "Reasoned" with its confidence, `ambition` as "A direction you're reaching for" with no confidence, and anything else not at all. `livedWorld.rejects` remains the live image-path avoid-clause source (`#rejectsDirection`), the scene writer still receives world, identity, and creative summaries rather than the grammar (stage 6), and no render evaluation examines the returned image. **Grammar content produced before step 3 is unreviewed model output**, steered only by the schema `description` guidance, since no synthesis instruction governs the artifact yet.

   **Recorded, not reconciled by editing: the schema and the instructions use overlapping words for register language.** The schema's `camera` description permits a register word such as documentary or editorial as shorthand that resolves to stated settings in the same entry. The step 3 instructions ban mood adjectives outright by name (cinematic, moody, dynamic, epic, atmospheric, dramatic, striking, evocative, elevated) while preserving the schema's allowance for genre register words under a stated test: delete the word, and if the entry is still complete the word was shorthand. The two are compatible under that reading, because a mood adjective names a feeling and never resolves to settings while a genre word can. They are recorded here because a reader meeting "register word may appear" in one file and "register adjectives are banned" in the other could reasonably read a contradiction. The schema is unchanged, per the convention that the shape holds the constraint and the instructions elaborate. If a run shows the model using the genre-word allowance as a loophole for mood language, that is a finding that revisits the schema sentence rather than an editing decision taken now.
