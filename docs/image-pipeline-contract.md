...
- Delta from this commit, the scene call describes a plain product and the client stops abandoning renders that are still working. `src/production/package.js` gains a `scenePass` compile mode, `src/production/service.js` sets it on the scene compile, and `app/app.js` changes how long recovery waits and what decides the failure state. Evidence and reasoning are in `docs/findings-2026-09-02-scene-placeholder-and-recovery.md`.
  ...
  **The scene pass is a compile mode, not a fork.** `compileBrandWorldImagePackage` takes `scenePass`, default false (L440). When it is true and the job names a product, the Product knowledge section body is the placeholder from `sceneProductPlaceholder` (L415) instead of `compileProductSectionForImage` (L643). That is the only difference between the two compiles: the scene pass names the product, asks for a plain unmarked version at its true physical size relative to hands, furniture, and surroundings, and says the real artwork is applied in a separate step. Nothing else in the compiler reads the flag, and at its default every compiled package is unchanged in every byte, checked against head across the four frozen fixture scene shapes locked and unlocked.
