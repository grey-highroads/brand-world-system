...
- Verified against commit: `963bdd0625c7b90244d4727bd45a499425c0199f`, the head this update was read against. The maintenance rule asks for this commit; a commit cannot carry its own SHA, so the parent is named and the deltas this commit introduces are stated below.
...
- Delta from this commit: documentation only. The ADR 0017 step 3 blob path is confirmed by its first real write and ambient state 16 is amended to say so. No module changed.
