...
| Generation package | User-facing preflight + durable execution artifact | See what will be sent, reuse it elsewhere, or correct a job-specific instruction | Show the compiled prompt, exclusions, recommended references, and output requirements before rendering; keep provenance and the original compiled version |
| Provider and model selection | Workflow configuration + contextual user choice | Decide whether and where to render when destinations create a meaningful tradeoff | Configure a default per workflow; expose export and authorized destination choice at the render boundary rather than as permanent controls |
...
Riggg exposed a third correction. Prompt compilation and rendering are distinct product stages. The system should present a portable generation package containing the compiled prompt, exclusions, and recommended references before invoking a renderer. Users may copy or export it, and authorized prompt edits remain scoped to the job and pass through policy validation again. Rendering uses a configured default while allowing meaningful destination choice or export-only completion. See [`decisions/0006-treat-generation-package-as-portable-artifact.md`](decisions/0006-treat-generation-package-as-portable-artifact.md).
