...
## Amendment, third ruling of 2026-08-31: authored exclusions restored

The reset cut the brief's per-job exclusions and the product record's exclusions from the scene prompt along with the governance recital, and additional finding 1 above recorded the loss. Both are restored as one compact avoid sentence at the end of the Protection block, in the order brief then product. The distinction that ruled the call: these are authored depiction intent, the user and the product record directing the frame, the same class of content as the scene text, while the recital was governed prohibition records, and the recital stays cut. The authored values compile byte verbatim, never summarized or reordered, which is also what lets the constraint audit's exact substring check find them: brief exclusions on the scene path now audit as carried again with no change to the audit code, and guardrails keep auditing as review, which stays honest because they still do not compile. (Verified by test assertion on the audited status.)
