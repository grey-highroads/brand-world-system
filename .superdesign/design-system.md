...
Source intake is type-first and accepts one file, URL, or written source per record. The concrete user-facing choices are Protected brand asset, Approved brand guidance, Past brand work or research, Single creative image, Image grid or moodboard, Named cultural reference, and Other business document. Do not expose the internal `brand-evidence` authority label. Protected assets stay exact and approved guidance is followed where relevant; neither receives an influence weight. Past work, research, and outside references can use Lead, Strong, Supporting, or Light influence, always explained as creative priority rather than a blend percentage. Every source requires a plain-language usage instruction and guidance area, with optional exclusions. File intake states its accepted formats, 20 MB per-file limit, 40 MB synthesis-batch limit, and that folders or mixed multi-file records are not accepted. Approved guidance accepts supported documents, structured text, and PNG, JPG, or WebP page images. Recommend PDF for multi-page books and explain that SVG, HEIC, TIFF, Keynote, and native design files need conversion before interpretation.

The declared material type is checked against file compatibility and contents. A mismatch is flagged for review rather than silently trusted. After Brand Brain approval, the Sources screen keeps the active version visually explicit: additions form a proposed update, only affected guidance is reconsidered, and the approved version remains available to production until the candidate is reviewed and approved.
